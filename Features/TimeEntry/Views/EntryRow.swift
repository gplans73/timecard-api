...
struct EntryRow: View {
    ...
    @State private var hoursText: String = "0"   // 👈 show 0 (not 0.00)
    ...
    private func syncTextFromHours() {
        hoursText = formatted(hours)              // 0 -> "0", 2.5 -> "2.5"
    }
                        ...
                        HStack(spacing: 6) {
                            ...
                            JobCodeTextField(title: "Job", text: $jobNumber)
                                ...
                                .onChange(of: jobNumberFocused) { _, nowFocused in
                                    if !nowFocused { jobNumber = jobNumber.trimmingCharacters(in: .whitespacesAndNewlines) }
                                }
                ...
                ZStack { // text field cell
                    ...
                    TextField("0", text: $hoursText)
                        ...
                        .onChange(of: hoursText) { _, _ in
                            // progressively sanitize but avoid heavy formatting each keystroke
                            var t = hoursText
                            if t.first == "." { t = "0" + t }
                            // prevent negative sign entry to reduce flicker
                            t.removeAll(where: { $0 == "-" })
