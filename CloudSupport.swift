...
enum CloudActions {
    ...
    static func syncNow() -> Bool {
        ...
        Task { @MainActor in
            CloudLog.append(ok ? "KVS synchronize() OK" : "KVS synchronize() FAILED")
        }
        return ok
    }
    ...
    static func syncAll(modelContext: ModelContext?, store: TimecardStore?) -> String {
        ...
        // iCloud KVS: check account, push snapshots, and synchronize with a short retry
        ...
        if FileManager.default.ubiquityIdentityToken == nil {
            ...
            if let store {
                ...
                // Mirror select settings commonly synced
            ...
            if !ok {
                // Small retry after a brief delay; KVS can be transient
                Thread.sleep(forTimeInterval: 0.2)
                ok = kvs.synchronize()
            }
            if !ok {
                let keyCount = kvs.dictionaryRepresentation.keys.count
                kvsSummary = "KVS sync=FAILED (keys=\(keyCount))"
            } else {
                kvsSummary = "KVS sync=OK"
            }
        }
