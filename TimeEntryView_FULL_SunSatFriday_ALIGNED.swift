        ...
        return HStack(spacing: 12) {
            DatePicker(
                ...
                selection: Binding(
                    ...
                    set: { newDate in
                        let cal = Calendar.current
                        let picked = cal.startOfDay(for: newDate)

                        // Determine the pay period containing the picked date using the Sun→Sat, Friday-payday rules
                        let period = OddPayPeriodCalc.period(containing: picked)

                        // Set the store's pay period start to this period's Sunday
                        store.weekStart = cal.startOfDay(for: period.start)

                        // Choose week index based on which week the picked date falls into
                        let daysFromStart = cal.dateComponents([.day], from: period.start, to: picked).day ?? 0
                        let idx = max(0, min(max(0, store.payPeriodWeeks - 1), daysFromStart / 7))
...
private struct DaySection: View {
    ...
    @State private var isPickerTransitioning: Bool = false
    ...
    private var dayTitle: String {
        ...
        if store.isStatHoliday(day) {
            if let name = store.holidayName(for: day), !name.isEmpty {
                return "STAT – \(name.uppercased()) \(base)"
            } else {
                return "STAT \(base)"
            }
        }
                    ...
                    if visibleEntries.isEmpty {
                        ...
                        ForEach(visibleEntries) { entry in
                            ...
                            SwipeToDeleteRow(onDelete: {
                                ...
                                EntryRow(
                                    jobNumber: Binding(get: { b.wrappedValue.jobNumber }, set: { newVal in
                                        if isPickerTransitioning { return }
                                        b.wrappedValue.jobNumber = newVal
                                    }),
                                    hours:    Binding(get: { b.wrappedValue.hours },      set: { newVal in
                                        if isPickerTransitioning { return }
                                        b.wrappedValue.hours = newVal
                                    }),
                                    code:     Binding(get: { b.wrappedValue.code },       set: { newVal in
                                        if isPickerTransitioning { return }
                                        b.wrappedValue.code = newVal
                                    }),
                                    notes:    Binding(get: { b.wrappedValue.notes },      set: { newVal in
                                        if isPickerTransitioning { return }
                                        b.wrappedValue.notes = newVal
                                    }),
                                    isOvertime: Binding(get: { b.wrappedValue.isOvertime }, set: { newVal in
                                        if isPickerTransitioning { return }
                                        b.wrappedValue.isOvertime = newVal
                                    }),
                                    isNightShift: Binding(get: { b.wrappedValue.isNightShift }, set: { newVal in
                                        if isPickerTransitioning { return }
                                        b.wrappedValue.isNightShift = newVal
                                    }),
                                    onDelete: {
                                        if isPickerTransitioning { return }
                                        if let idx = store.entries.firstIndex(where: { $0.id == entry.id }) {
                                ...
                                .padding(.horizontal)
                                .id(entry.id) // stabilize identity to avoid rebuild during focus changes
                                ...
                                .onReceive(NotificationCenter.default.publisher(for: UIResponder.keyboardWillShowNotification)) { _ in
                                    // small guard during keyboard transitions
