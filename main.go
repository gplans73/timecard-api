...
func forceRecalcAndStripCalcChain(xlsx []byte) ([]byte, error) {
	// Excel can refuse to calculate (or can show stale results) when a workbook contains a calcChain
	// generated by a different engine. The safe approach is:
	//  1) Remove xl/calcChain.xml
	//  2) Remove its relationship entry from xl/_rels/workbook.xml.rels
	//  3) Remove the content-type override for calcChain from [Content_Types].xml
	//  4) Set workbook calcPr to fullCalcOnLoad="1" and calcMode="auto"
	//
	// This function rewrites the XLSX (ZIP) in-memory.
	...
	for _, f := range r.File {
		...
		case "xl/_rels/workbook.xml.rels":
			// 2) Remove calcChain relationship
			re := regexp.MustCompile(`<Relationship[^>]*Type="[^"]*/calcChain"[^>]*/>`)
			b = re.ReplaceAll(b, []byte(""))
		case "[Content_Types].xml":
			// 3) Remove content type override for calcChain.xml
			re := regexp.MustCompile(`<Override[^>]*PartName="/xl/calcChain\.xml"[^>]*/>`)
			b = re.ReplaceAll(b, []byte(""))
		case "xl/workbook.xml":
			// 4) Force full recalculation on load
			b = ensureCalcPrAutoFullCalcOnLoad(b)
		}

		// Preserve the original header where possible
		hdr := f.FileHeader
...
func ensureCalcPrAutoFullCalcOnLoad(workbookXML []byte) []byte {
	s := string(workbookXML)

	// Match the calcPr self-closing tag, e.g. <calcPr calcId="191025"/>
	re := regexp.MustCompile(`<calcPr([^>]*)/>`)
	m := re.FindStringSubmatchIndex(s)
	if m == nil {
		// If there's no calcPr, inject one before </workbook>
		insert := `<calcPr calcId="0" calcMode="auto" fullCalcOnLoad="1"/>`
	...
	// Keep calcId if present; otherwise default to 0
	calcID := "0"
	if mm := regexp.MustCompile(`calcId="([^"]*)"`).FindStringSubmatch(attrs); len(mm) == 2 {
		calcID = mm[1]
	}

	repl := fmt.Sprintf(`<calcPr calcId="%s" calcMode="auto" fullCalcOnLoad="1"/>`, calcID)
	s = s[:m[0]] + repl + s[m[1]:]
	return []byte(s)
}
...
func generateExcelFile(req TimecardRequest) ([]byte, error) {
	...
	out := buffer.Bytes()
	if fixed, err := forceRecalcAndStripCalcChain(out); err == nil {
		out = fixed
	} else {
		log.Printf("WARNING: post-process (recalc/strip calcChain) failed: %v", err)
	}
	return out, nil
}
