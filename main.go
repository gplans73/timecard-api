...
func forceRecalcAndRemoveCalcChain(xlsx []byte) ([]byte, error) {
	...
	for _, zf := range zr.File {
		...
		if needsModification {
			...
			// For files we don't modify (like styles.xml), preserve original compression method exactly
			// Copy the file header and use the same compression method
			hdr := zf.FileHeader
			// CRITICAL: Preserve the original compression method to avoid corrupting styles.xml
			// Don't change Method - keep it as it was in the original file
			w, err := zw.CreateHeader(&hdr)
			if err != nil {
				_ = zw.Close()
				return nil, fmt.Errorf("write %s: %w", name, err)
