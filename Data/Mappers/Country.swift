...
final class TimecardStore: ObservableObject {
    ...
    @Published var payPeriodNumber: Int = 39
    ...
    func clonedStoreForPayPeriod(start: Date) -> TimecardStore {
        ...
        clone.payPeriodNumber = (weekOfYear / 2) + 1
    ...
    init() {
        ...
        // Load or seed Jobs list (independent from labour codes)
        if storedJobsJSON.isEmpty {
            // Seed jobs from official labour codes on first run; can diverge later
        ...
        // One-time override from bundled JobsSeed.csv if present
